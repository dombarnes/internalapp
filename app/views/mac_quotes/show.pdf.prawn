pdf = Prawn::Document.new(:info => { :Title => "Mac Quote for #{@mac_quote.customer}", :Author => "MacByte Ltd", :Creator => "MacByte Ltd", :CreationDate => Time.now}
) do
...
pdf.font "Helvetica"
pdf.font_size 18
pdf.text "Mac Installation and Support Quote for #{@mac_quote.customer}", :align => :right
pdf.move_down 2
pdf.font_size 14
...
quoteinfo = [["Customer Name", "#{@mac_quote.customer}"], ["Device Quantity", "#{@mac_quote.device_quantity}"],["Installation Cost", "#{number_to_currency(@mac_quote.install_cost, :unit => "£")}"],["Support Cost", "#{number_to_currency(@mac_quote.support_cost, :unit => "£")}"]]
